...
import { getUsers, addUser } from "../dist/users.js";

const require = createRequire(import.meta.url);
const mockUsers = require("./mocks/users.js");
...
export const handler = async (event) => {
  ...
  try {
    ...
    const path =
      event.path || event.resource || event.pathParameters?.proxy || "";

    // GET /users
    if (
      event.httpMethod === "GET" &&
      (path === "/users" || path.includes("/users"))
    ) {
